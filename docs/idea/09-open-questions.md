...
| Should touch-and-hold, pattern swipes, and per-axis (X-only/Y-only) swipe be added as new calibration primitives, or stay out of scope? | Raised in [20 — Meeting Notes: Input Modes](20-meeting-notes-input-modes.md); none of these are in the current §3.1 calibration test set |
| Should the `sounds` voice-clarity tier be upgraded from binary vocalization-presence to a sound-count vocabulary (e.g. one sound = yes, two = no)? | Same meeting; would reuse the existing VAD pipeline ([tech/research/02](../tech/research/02-speech-recognition.md) §4) but needs burst-counting logic added |
| Is grid-based voice selection worth building for the demo, given Android Voice Access already ships the exact pattern? | See [tech/research/10 — Additional Input Modes](../tech/research/10-additional-input-modes.md) §3 — feasibility is there, but not yet weighed against the locked demo scope in [05 — Locked Scope](05-scope.md) |

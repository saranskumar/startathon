...
**App status (Sept 2026):** the tunnel *overlay* (yellow square + dimmed outside) is being removed. Real tunnel layout is further scope: once built, all buttons, text, and options live inside that square. See [app/04](../app/04-clarity-playground-and-field.md).
